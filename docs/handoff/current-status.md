...
- The prompt engine also suppresses centered foreground gimmicks. Obstructions should be edge/corner interruptions that leave the ordinary location readable, not hands or plastic bags replacing people as the main subject.
- Roll DNA now assigns a deterministic hidden camera profile from the resolved situation. Profiles cover ordinary compact, disposable-camera, instant-camera, Lomo-like compact, and cheap point-and-shoot behavior, then prompt assembly exposes only physical capture traits such as soft edges, weak flash, mild color casts, loose focus, and muted print color.
- Person handling should avoid both extremes: no centered portrait subjects, but also no repeated artificial pattern where everyone is turned away. The current prompt allows small/off-center/soft front-facing, three-quarter, profile, looking-down, or partly blocked people.
- `_sample/` is a local, ignored reference-photo directory and must not be committed. Its current abstraction is now represented in the prompt engine as place-led/surface-led frames: shopfronts, signs, shelves, vending machines, chairs, walls, windows, road markings, tables, packaging, numbers, and empty space can carry a frame without becoming product photography or graphic-design studies.
- `Tiny Detail` must remain an optional clue, not a required foreground object. This is especially important for US cup/drink/tray details, which previously made every frame repeat a front tumbler-like prop.
- Contact-sheet generation now calls the image provider per frame instead of using one shared `n: 8` request. Each frame has a hidden role/fault/tiny-detail/aspect plan, including deliberately plain or failed exposures, to reduce repeated AI motifs.
- Remote contact-sheet frames now mix `1536x1024`, `1024x1536`, and `1024x1024` provider sizes. The DB already stores width/height per frame; keep UI changes aware that future generated frames are not always square.
- `write_generated_png` lightly post-processes provider PNGs before saving with weak softness, deterministic micro-noise, lifted blacks, and subtle edge falloff. This should stay subtle and should not become an obvious retro filter.
